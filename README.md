...
| `r` | Принудительное обновление ping для всех серверов |
| `q` | Выход |
| `Ctrl+C` / `SIGTERM` | Выход (graceful shutdown) |
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│ 🌐 NetWatch  │  13:45:21  │  🌍 91.108.4.12 (Moscow, RU Russian Federation,  │
│              │              │  55°N 37°E)                                    │
├──────────────────────────────────────────────────────────────────────────────┤
...
├──────────────────────────────────────────────────────────────────────────────┤
│ ↑/↓: Select | r: Refresh | Enter: Detail | q: Quit                           │
└──────────────────────────────────────────────────────────────────────────────┘
```

### Внешний IP

В заголовке отображается полная информация о вашем внешнем IP:

```
🌍 91.108.4.12 (RU Russian Federation, Moscow, 55.7°N 37.6°E)
```

**Данные получаются с ipwho.is:**
- IP адрес
...
Программа корректно завершает работу при:
- Нажатии `q` в любом режиме
- Получении сигнала `SIGTERM` (`kill -TERM <pid>`)
- Получении сигнала `SIGINT` (`Ctrl+C`)

**Примечание:** Из-за особенностей `crossterm` в raw mode, `Ctrl+C` может блокироваться на ~100ms. Для гарантированного завершения используйте `kill -TERM <pid>`.
